...
public class RegistrationActivity extends AppCompatActivity {
	TextInputEditText email, password, name, number;
	String id, pass, n, num;
	...
	public void reg(View v) {
		...
		num = Objects.requireNonNull(number.getText()).toString().trim();
		if (n.isEmpty()) {
			name.setError("empty");
			name.requestFocus();
			...
			mAuth.createUserWithEmailAndPassword(id, pass).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
				...
				public void onComplete(@NonNull Task<AuthResult> task) {
					if (task.isSuccessful()) {
						...
						createuser(n, user != null ? user.getEmail() : null, "", num);
						pd.dismiss();
	...
	public void createuser(String name, String email, String imgurl, String number) {
		db.push();
		Createuser user = new Createuser(email, String.valueOf(maxid + 1), imgurl, name, number);
